...
// Retorno tipado como never ou GetUploadsOutput.
// Se não tiver retorno, significa que a função não retorna nada e não um é um erro.
// Se o retorno for GetUploadsOutput, função retorna um objeto com as propriedades.
export async function getUploads(input: GetUploadsInput): Promise<Either<never, GetUploadsOutput>> {
  ...
  const [uploads, [{ total }]] = await Promise.all([
    db
      ...
      .offset((page - 1) * pageSize) // offset= nº de registros pulados. See page = 1, offset = 0. Se page = 2, offset = 20. Se page = 3, offset = 40.
      .limit(pageSize),

    db
      .select({ total: count(schema.uploads.id) })
      .from(schema.uploads)
      .where(searchQuery ? ilike(schema.uploads.name, `%${searchQuery}%`) : undefined), //ilike = case insensitive.
  ])
